...
func SaveH2HRecord(team1, team2 string, totalMatches, teamAWins, draws, teamBWins int, avgAGoals, avgBGoals float64) error {
	teamKey, _, _ := GetSortedTeamKey(team1, team2)
	query := `INSERT OR REPLACE INTO h2h_records 
              (team_key, total_matches, team_a_wins, draws, team_b_wins, avg_a_goals, avg_b_goals, last_updated)
              VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`
	_, err := DB.Exec(query, teamKey, totalMatches, teamAWins, draws, teamBWins, avgAGoals, avgBGoals)
	return err
}

// GetH2HRecord 读取本地两队的历史交手统计数据
func GetH2HRecord(team1, team2 string) (totalMatches, teamAWins, draws, teamBWins int, avgAGoals, avgBGoals float64, found bool, err error) {
	teamKey, _, _ := GetSortedTeamKey(team1, team2)
	query := `SELECT total_matches, team_a_wins, draws, team_b_wins, avg_a_goals, avg_b_goals 
              FROM h2h_records WHERE team_key = ?`
	err = DB.QueryRow(query, teamKey).Scan(&totalMatches, &teamAWins, &draws, &teamBWins, &avgAGoals, &avgBGoals)
	if err == sql.ErrNoRows {
	...
	return totalMatches, teamAWins, draws, teamBWins, avgAGoals, avgBGoals, true, nil
}
