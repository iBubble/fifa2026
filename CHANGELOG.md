...
### Changed
- **多场混合过关精算成功率最大化与自适应降级重构**：
  - 重构了 `parlay.go` 中的 `getBestSingleChoice` 选择算法。所有串关玩法（胜平负、让球、半全场、总进球数、比分）的单场选项筛选逻辑从原本的“期望价值最大化（Max EV）”变更为“成功率最大化（Max Probability）”。
  - **自适应混合玩法降级**：升级了半全场（hafu）、总进球（ttg）及比分（crs）的停售拦截逻辑。若某场比赛未售该类玩法，系统自动降级切换至该场已售的常规胜平负（had）或让球胜平负（hhad）以代替之，保证依然能凑足场数组成高成功率的混合串关推荐，防止玩法卡片因场数不足被隐藏。
  - 彻底解决了由于爆冷高赔选项强占高 EV 导致多场串关组合出成功率极低（低于 1%）的废单问题，保证系统推荐的都是中奖几率最高的黄金正路组合。
- **未开售玩法全面“不可售”熔断拦截**：
